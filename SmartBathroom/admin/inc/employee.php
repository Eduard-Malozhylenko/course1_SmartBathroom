...
$errMsg = $id_organization = $id = $login = $password = $full_name = $status = '';
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(empty($_POST['id_organization']) or empty($_POST['login']) or empty($_POST['password']) or empty($_POST['full_name']) or empty($_POST['status'])){
        ...
        $str = "id_organization={$_POST['id_organization']}&login={$_POST['login']}&password={$_POST['password']}&full_name={$_POST['full_name']}&status={$_POST['status']}";
        ...
        if(!empty($_POST['id'])){
            ...
            $result = json_decode(curl_exec($curl));
            if($result->status){
                header('Location: /admin/employee.php');exit;
            }else{
...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
    id органызації:<br />
    <input type="text" name="id_organization" value="<?=$id_organization?>" /><br />
    Логін:<br />
...
if($id)
    exit;
?>
<h3>Всього працівників: <?=count($result)?></h3>
<?php
...
foreach($result as $empl){
	...
	<p align="right">
		<a href="/admin/employee.php?del={$empl->id}">Видалити</a>&nbsp;
		<a href="/admin/employee.php?update={$empl->id}">Змінити дані</a>
	</p>	
BOOK;
}
?>
